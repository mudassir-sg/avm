...
static INLINE void print_partition_timing_stats(
    const PartitionTimingStats *part_timing_stats, const int bsize,
    const int mi_row, const int mi_col, const char *filename) {
...
static AVM_INLINE PC_TREE *const *get_child_pc_trees(
    const PC_TREE *pc_tree, const PARTITION_TYPE partition,
    const REGION_TYPE region_type, int *num_sub_parts) {
  PC_TREE *const *child_nodes = NULL;

  switch (partition) {
    case PARTITION_NONE: *num_sub_parts = 0; break;
    case PARTITION_HORZ:
      child_nodes = pc_tree->horizontal[region_type];
      *num_sub_parts = SUB_PARTITIONS_RECT;
      break;
    case PARTITION_VERT:
      child_nodes = pc_tree->vertical[region_type];
      *num_sub_parts = SUB_PARTITIONS_RECT;
      break;
    case PARTITION_HORZ_3:
      child_nodes = pc_tree->horizontal3[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_VERT_3:
      child_nodes = pc_tree->vertical3[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_HORZ_4A:
      child_nodes = pc_tree->horizontal4a[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_HORZ_4B:
      child_nodes = pc_tree->horizontal4b[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_VERT_4A:
      child_nodes = pc_tree->vertical4a[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_VERT_4B:
      child_nodes = pc_tree->vertical4b[region_type];
      *num_sub_parts = 4;
      break;
    case PARTITION_SPLIT:
      child_nodes = pc_tree->split[region_type];
...
static AVM_INLINE void get_partition_subblock_layout(
    const PARTITION_TYPE partition_type, const BLOCK_SIZE bsize,
    const int mi_row, const int mi_col, BLOCK_SIZE sub_sizes[4], int mi_rows[4],
    int mi_cols[4]) {
  ...
  for (int i = 0; i < subblock_count[partition_type]; ++i) {
    mi_rows[i] =
        mi_row + step_multiplier[partition_type][i][0] * eighth_step_h / 4;
    mi_cols[i] =
        mi_col + step_multiplier[partition_type][i][1] * eighth_step_w / 4;
    sub_sizes[i] = part_subsize;
  }
...
static AVM_INLINE void trace_partition_boundary(bool *partition_boundaries,
  ...
  if (partition == PARTITION_NONE) {
    ...
    for (int i = 0; i < num_sub_parts; ++i) {
      trace_partition_boundary(partition_boundaries, child_nodes[i], mi_rows[i],
                               mi_cols[i], subblock_sizes[i]);
    }
  }
}
  ...
  if (pc_tree->partitioning == PARTITION_NONE) {
    ...
    *total_count += 1;
    ...
    PC_TREE *const *child_nodes = get_child_pc_trees(
        pc_tree, pc_tree->partitioning, pc_tree->region_type, &num_sub_parts);
    for (int i = 0; i < num_sub_parts; ++i) {
      early_termination_inter_sdp(child_nodes[i], total_count,
                                  inter_mode_count);
    }
  }
}
...
static void search_extended_partition(
  ...
  if (rect_type == HORZ) {
    ...
    if (!(search_state->do_rectangular_split ||
      ...
      return;
    if (is_h_partition) {
      // TODO(yuec): set default partition modes for the edge directly by ruling
      // out h partitions from the syntax if the 2nd middle block is not in the
      // frame.
  ...
  int num_sub_parts = 0;
  PC_TREE *const *child_nodes_read_only = get_child_pc_trees(
      pc_tree, partition_type, pc_tree->region_type, &num_sub_parts);
  PC_TREE **child_nodes = (PC_TREE **)child_nodes_read_only;
  ...
  for (int idx = 0; idx < num_sub_parts; ++idx) {
    if (child_nodes[idx]) {
      av2_free_pc_tree_recursive(child_nodes[idx], num_planes, 0, 0);
      child_nodes[idx] = NULL;
    }
    child_nodes[idx] = av2_alloc_pc_tree_node(
        xd->tree_type, mi_rows[idx], mi_cols[idx], cm->sb_size,
        subblock_sizes[idx], pc_tree, partition_type, idx,
        idx == (num_sub_parts - 1), ss_x, ss_y);

    child_nodes[idx]->is_cfl_allowed_for_this_chroma =
        pc_tree->is_cfl_allowed_for_this_chroma |
        is_cfl_allowed_for_this_chroma_partition;
  }
  bool skippable = true;
  for (int i = 0; i < num_sub_parts; ++i) {
    const int this_mi_row = mi_rows[i];
    const int this_mi_col = mi_cols[i];
    ...
    SUBBLOCK_RDO_DATA rdo_data = { child_nodes[i],
                                   get_partition_subtree_const(ptree_luma, i),
...
static AVM_INLINE int get_partition_depth(const PC_TREE *pc_tree,
  ...
  for (int i = 0; i < num_sub_parts; ++i) {
    max_depth = AVMMAX(
        max_depth, get_partition_depth(child_nodes[i], curr_depth + depth_inc));
  }
  return max_depth;
}
...
static AVM_INLINE void set_sms_tree_partitioning(
    SIMPLE_MOTION_DATA_TREE *sms_tree, const PARTITION_TYPE partition) {
...
static AVM_INLINE int get_ext_partitions_recur_depth(const AV2_COMP *const cpi,
                                                     const BLOCK_SIZE bsize) {
  const int level = cpi->sf.part_sf.ext_recur_depth_level;
...
static void prune_partitions_using_ml_results(
    ...
    PartitionSearchState *part_search_state, const int search_none_after_rect,
    const int force_prune_flags[MAX_PRUNE_TYPES],
...
BEGIN_PARTITION_SEARCH:
  ...
  int next_force_prune_flags[NUM_RECT_PARTS][MAX_PRUNE_TYPES] = { { 0, 0, 0 },
                                                                  { 0, 0, 0 } };
  ...
  if (search_none_after_split && pc_tree->partitioning == PARTITION_SPLIT) {
    for (int idx = 0; idx < SUB_PARTITIONS_SPLIT; idx++) {
      const int depth =
          get_partition_depth(pc_tree->split[pc_tree->region_type][idx], 0);
      if (depth != 0) {
        search_none_after_split = 0;
        search_none_after_rect = 1;
        break;
      }
    }
  }
  ...
  const int ext_recur_depth = AVMMIN(
      max_recursion_depth - 1, get_ext_partitions_recur_depth(cpi, bsize));
  ...
  if ((pc_tree->region_type != INTRA_REGION || frame_is_intra_only(cm))) {
    ...
    for (PARTITION_TYPE partition_type = PARTITION_HORZ_4A;
      ...
      prune_4way_partitions_based_on_4way_search(
          cpi, pc_tree, &part_search_state, partition_type);
    }
  }
